...
import { getTypes } from "../../assets/types/types";
...
const DetailsPage = () => {
  ...
  const type = () => {
    ...
    return <img src={getTypes(pokemonDetails?.data?.types[0]?.type?.name)} />;
  };
    ...
    <>
      ...
      <ChakraProvider>
        <Main>
          ...
          <Container
            color={getColors(pokemonDetails.data?.types[0]?.type?.name)}
          >
            ...
            <Moves>
              <div className="informations-id">
                {pokemonDetails.data?.id <= 9 ? (
                  <div>#0{pokemonDetails.data?.id}</div>
                ) : (
                  <div>#{pokemonDetails.data?.id}</div>
                )}
              </div>
              <div className="informations-name">
                {pokemonDetails.data?.name.charAt(0).toUpperCase() +
                  pokemonDetails.data?.name.slice(1)}
              </div>
              <div className="informations-type">{type()}</div>
              <div className="background">
                <text className="text">Moves:</text>
                {pokemonDetails.data?.moves
                  ?.filter((mov, index) => index <= 3)
                  .map((moves) => {
                    return (
