...
export default function HeaderDetails(pokemon) {
  ...
  return (
    <Box bg="white" w="100%" h="10rem">
      <Grid templateColumns="repeat(3, 1fr)" gap={"0.5rem"} ml="1.75rem">
        <Link to={"/"}>
          <Button
            colorScheme="white"
            size="md"
        ...
        <Image src={PokemonLogo} alt="Pokemon Logo" ml="5.0rem" mt="0.9rem" />
        {/* {location.pathname===`/pokemon/${pokemon.name}` || pokelist.find((item) => pokemon === item.data.id) ? (
          <Button color="white" bg="#FF6262" w="225px" h="58px" mt="30px" onClick={()=> removePokemon(pokemon, onOpen, onClose)}>
            Excluir da Pokédex
          </Button>
        ) : (
          <Button color="white" bg="#FF6262" w="225px" h="58px" mt="30px" onClick={()=>addPokemon(pokemon, onOpen ,onClose)}>
